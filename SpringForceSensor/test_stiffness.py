...
probe_mass = 3.5
sensor = SpringForceSensor("/World/Sensor", probe_mass = probe_mass)
...
try:
    ...
    while simulation_app.is_running():
        ...
        if i%100 == 0:
            ...
            measured_f = sensor.get_equivalent_force_geom()

            # print(f"传感器力: {measured_f} N")
            # print(f"mass = {probe_mass} 传感器力: N:\n")
            for f in measured_f:
                print(f"{f:.6f}", end=',')
            print(f"{probe_mass}")
